...
if(isset($_POST["submit"])){
    ...
    if (mysqli_num_rows($query) == 0){
        $reqs = false;
        echo '<div class="alert">' . 'Invalid Instructor ID' . '</div>';
    ...
    if (mysqli_num_rows($timeslotQuery) >= 2){
        $reqs = false;
        echo '<div class="alert">' . 'Two sections are already assigned to this timeslot' . '</div>';
    ...
    if (mysqli_num_rows($secQuery) > 0){
        $reqs = false;
        echo '<div class="alert">' . 'his section already exists' . '</div>';
    ...
    if (mysqli_num_rows($instQuery) > 2){
        $reqs = false;
        echo '<div class="alert">' . 'This instructor already teaches two sections' . '</div>';
    ...
    if (mysqli_num_rows($iQuery) == 1){
        ...
        if ($curDay != $day){
            $reqs = false;
            echo '<div class="alert">' . 'The instructor must have two consecutive timeslots' . '</div>';
        ...
        // if the timeslots are consecutive on the same day, accept and insert into section
            ...
            echo '<div class="msg">' . 'Section created successfully' . '</div>';
